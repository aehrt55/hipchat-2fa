...
function serviceProviderLogo(serviceProvider) {
  ...
  case 'Kickstarter':
    return 'https://authy.tenten.co/img/KickStarter_Logo.png';
  default:
    return 'https://authy.tenten.co/img/DigitalOcean_logo.jpg';
  }
}
...
function token() {
  ...
  Secret.findOne(query, function(err, secret) {
    try {
      ...
      var service = secret.serviceProvider || secret.issuer;
      var serviceImage = serviceProviderLogo(service);
      cb(null, {
        message: token,
        card: {
          style: 'link',
...
module.exports.serviceProviderLogo = serviceProviderLogo;
